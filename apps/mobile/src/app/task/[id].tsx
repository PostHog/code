...
export default function TaskDetailScreen() {
  ...
  // Refresh task title periodically — the server often generates a better
  // title after the agent starts working.
  useEffect(() => {
    if (!taskId || loading) return;
    const interval = setInterval(() => {
      getTask(taskId)
        .then((fresh) => {
          if (fresh.title && fresh.title !== task?.title) {
            setTask((prev) => (prev ? { ...prev, title: fresh.title } : prev));
          }
        })
        .catch(() => {});
    }, 10_000);
    return () => clearInterval(interval);
  }, [taskId, loading, task?.title]);
