...
const SYSTEM_PROMPT = `You are summarizing the result of an automated MCP query that found information relevant to someone working at a tech company.

Rules:
- Produce no more than 4 bullet points
- Each bullet should convey the most important or critical detail — what matters to the reader's work
- Do NOT just list statistics about what was scanned or checked
- Give a qualitative sense of what is important, so the reader can decide whether to expand and read the full text without wasting their time
- If nothing critical was found, say so in one line
- Use markdown formatting (bold for emphasis, links if present in the source)
...
export function useSummary(
  ...
  const generate = useCallback(async (content: string) => {
    ...
    if (!auth.oauthAccessToken || !apiHost) {
      ...
      const fallback = sep !== -1 ? content.slice(0, sep).trim() : content.slice(0, 500);
