...
export function TemplateLibrary({ onApply }: TemplateLibraryProps) {
  ...
  const userTemplates = useAutomationStore((s) => s.userTemplates);

  const allTemplates = useMemo(
    () => [...AUTOMATION_TEMPLATES, ...userTemplates],
    [userTemplates],
  );
  ...
  const CATEGORIES = useMemo(
    () => [
      "All",
      ...new Set([...BASE_CATEGORIES, ...userTemplates.map((t) => t.category)]),
    ],
    [userTemplates],
  );

  const recommended = useMemo(
    () => allTemplates.filter((t) => t.recommended),
    [allTemplates],
  );
    ...
    <Flex direction="column" gap="3">
      ...
      <Flex direction="column" gap="2">
        ...
        <Flex gap="3" wrap="wrap">
          ...
          {filtered.length === 0 && (
            <Text size="1" className="py-4 text-gray-10">
